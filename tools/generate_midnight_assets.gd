...
const INK := Color(0.016, 0.025, 0.030, 1)
const TEAL := Color(0.060, 0.245, 0.225, 1)
const MINT := Color(0.35, 0.95, 0.75, 1)
const GOLD := Color(1.00, 0.66, 0.22, 1)
const COPPER := Color(0.82, 0.34, 0.18, 1)
const VIOLET := Color(0.60, 0.42, 1.00, 1)
const ROSE := Color(1.00, 0.22, 0.32, 1)
const SKY := Color(0.20, 0.76, 0.96, 1)
const GLASS := Color(0.72, 0.96, 1.00, 0.72)
const WHITE := Color(0.96, 1.00, 0.92, 1)
...
func _init() -> void:
	...
	_save_coin("res://textures/coins/coin_normal.png", GOLD, COPPER, "normal")
	_save_coin("res://textures/coins/coin_lucky.png", Color(0.42, 0.95, 0.58), GOLD, "lucky")
	...
	_save_coin("res://textures/coins/coin_stock.png", Color(0.20, 0.84, 0.70), GOLD, "stock")
	_save_coin("res://textures/coins/coin_bitcoin.png", Color(0.95, 0.55, 0.18), GOLD, "bit")
...
func _save_coin(path: String, body: Color, accent: Color, symbol: String) -> void:
	...
	_draw_ellipse(img, center + Vector2(0, 18), Vector2(190, 94), Color(0, 0, 0, 0.30))
	_draw_ellipse(img, center, Vector2(190, 104), body.darkened(0.32))
	_draw_ellipse(img, center + Vector2(0, -5), Vector2(176, 90), body)
	_draw_ellipse(img, center + Vector2(-34, -30), Vector2(94, 32), _alpha(body.lightened(0.26), 0.36))
	_draw_ellipse_outline(img, center, Vector2(190, 104), accent.lightened(0.15), 9)
	_draw_ellipse_outline(img, center, Vector2(140, 70), _alpha(INK, 0.32), 4)
	_draw_ellipse_outline(img, center, Vector2(118, 58), accent.lightened(0.35), 3)
...
func _save_tile(path: String, symbol: String) -> void:
	...
	_draw_round_rect(img, Rect2(54, 54, 404, 404), 72, Color(0, 0, 0, 0.28))
	_draw_round_rect(img, Rect2(48, 42, 416, 416), 72, TEAL.darkened(0.22))
	_draw_round_rect(img, Rect2(72, 66, 368, 368), 56, TEAL)
	_draw_round_rect_outline(img, Rect2(48, 42, 416, 416), 72, MINT.darkened(0.18), 12)
	_draw_round_rect_outline(img, Rect2(92, 88, 328, 328), 44, Color(1, 1, 1, 0.12), 5)
...
func _save_icon(path: String, accent: Color, symbol: String) -> void:
	...
	_draw_round_rect(img, Rect2(14, 18, 100, 100), 26, Color(0, 0, 0, 0.30))
	_draw_round_rect(img, rect, 26, TEAL.darkened(0.25))
	_draw_round_rect(img, Rect2(18, 18, 92, 92), 22, TEAL)
	_draw_round_rect_outline(img, rect, 26, accent, 5)
	match symbol:
		"enemy":
...
func _save_enemy_icon(path: String, accent: Color, symbol: String) -> void:
	...
	var center := Vector2(64, 64)
	_draw_round_rect(img, Rect2(13, 17, 104, 104), 28, Color(0, 0, 0, 0.32))
	_draw_round_rect(img, rect, 28, TEAL.darkened(0.28))
	_draw_round_rect(img, Rect2(18, 18, 92, 92), 23, TEAL.darkened(0.02))
	_draw_round_rect_outline(img, rect, 28, accent, 5)
...
func _save_item_icon(path: String, accent: Color, symbol: String) -> void:
	...
	var rect := Rect2(8, 8, 112, 112)
	var center := Vector2(64, 64)
	_draw_round_rect(img, Rect2(13, 17, 104, 104), 28, Color(0, 0, 0, 0.30))
	_draw_round_rect(img, rect, 28, TEAL.darkened(0.26))
	_draw_round_rect(img, Rect2(18, 18, 92, 92), 23, TEAL)
	_draw_round_rect_outline(img, rect, 28, accent, 5)
